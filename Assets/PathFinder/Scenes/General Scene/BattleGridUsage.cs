...
namespace K_PathFinder.Samples {
    ...
    public class BattleGridUsage : MonoBehaviour {
        ...
        void Start() {
            ...
            _agent.SetRecieveSampleDelegate(RecievePointsDelegate, AgentDelegateMode.ThreadSafe);//to sure it's in main thread
        ...
        IEnumerator UpdateBattleGridCall() {
            while (true) {
                _agent.SetGoalGetSamplePoints(25);
                yield return new WaitForSeconds(0.25f);
            }
        }


        void RecievePointsDelegate(IEnumerable<PointQueryResult<CellSamplePoint>> points) {
            //take some values
            ...
            CellSamplePoint? targetPoint = null;
            ...
            foreach (var point in points) {
                ...
                for (int v = 0; v < _heightTests.Count; v++) {
                    if (Physics.Linecast(testTarget, point.value.position + _heightTests[v] * agentHeight * Vector3.up, layers) == false) {
                ...
                value *= Mathf.Clamp01((10 - Vector3.Distance(transform.position, point.value.position)) / 10);
                ...
                Vector3 cameraKeyPoint = Vector3.Cross(cam.transform.position - point.value.position, Vector3.up).normalized;
                meshVerts[debugIndex * 4] = point.value.position + (cameraKeyPoint * -debugWidth); //LB
                meshVerts[debugIndex * 4 + 1] = point.value.position + (cameraKeyPoint * debugWidth); //RB
                meshVerts[debugIndex * 4 + 2] = point.value.position + (cameraKeyPoint * -debugWidth) + new Vector3(0, value * debugHeight, 0); //LT
                meshVerts[debugIndex * 4 + 3] = point.value.position + (cameraKeyPoint * debugWidth) + new Vector3(0, value * debugHeight, 0); //RT
            ...
            if (targetPoint != null)
                _agent.SetGoalMoveHere(targetPoint.Value);
        }
    }
}
